...
const preEpoch_deadlineParser = (deadline) => {
  const hours = (Date.parse(deadline) - Date.now()) / (1000 * 60 * 60) + 24;
  return hours > 0 ? Math.round(hours) : 0;
};

export const preEpoch_mergeCourseData = (user_data, duty_data, course_id, course_name = '') => new Promise((resolve, reject) => {
  //
  ...
  for (let i = 0; i < preTasks.length; ++i) {
    const task_status = preTasks[i].taskExpired.statusID === null ? 0 : preTasks[i].taskExpired.statusID;
    tasks.push({
...
export const preEpoch_getCurrentTaskData = (task_id, courses) => {
  for (const item of courses) {
    for (const task of item.tasks) {
      if (task.courseTaskID === task_id) { return task; }
    }
  }
};
...
export const preEpoch_saveCourses = (old_c, new_c) => new Promise((saveRes, saveRej) => {
  ...
  const findCourse = (course_id) => {
    for (const item of old_c) {
      if (course_id === item.course_id) { return { status: 1, course: item }; }
    }
